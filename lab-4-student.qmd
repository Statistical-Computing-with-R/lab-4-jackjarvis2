...
```{r}
#| label: recreate-plot
# Filter the data for the relevant years and categories
plot_data <- ca_childcare %>%
  ...
  gather(key = "age_group", value = "median_price", mc_infant:mc_preschool)

# Create the plot
ggplot(plot_data, aes(x = study_year, y = median_price, color = census_region)) +
  geom_line() +
  facet_wrap(~ age_group, scales = "free_y", ncol = 1, labeller = as_labeller(c(
    mc_infant = "Infants",
    mc_toddler = "Toddlers",
